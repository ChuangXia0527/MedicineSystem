    ...
    <el-main>
      ...
      >
        <el-form-item label="编号">
          <el-input v-model="searchForm.mno" placeholder="请输入编号" clearable></el-input>
        </el-form-item>
        <el-form-item label="药品名称">
          <el-input v-model="searchForm.mname" placeholder="请输入药品名称" clearable></el-input>
        </el-form-item>
        <el-form-item label="服用方法">
          <el-select v-model="searchForm.mmode" placeholder="请选择服用方法" clearable>
            <el-option label="内服" value="内服"></el-option>
            <el-option label="外用" value="外用"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="药品功效">
          <el-input v-model="searchForm.mefficacy" placeholder="请输入药品功效" clearable></el-input>
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="服用方法" width="150">
          <template #default="scope">
            <span>{{ scope.row.mmode }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="mefficacy" label="药品功效" width="450"></el-table-column>
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="scope">
            <el-button
                @click="editMedicine(scope.row)"
                type="text"
                size="small"
                style="color: #409eff;"
                v-role="'管理员'"
            ...
            <template v-if="clients.some(client => client.mno === scope.row.mno)">
              <el-button type="text" size="small" disabled style="color: grey;" v-role="'管理员'">不可删除</el-button>
            </template>
            <template v-else>
              <el-button
      ...
      <el-dialog
          ...
          v-role="'管理员'"
      >
        <el-form :model="currentMedicine" label-width="100px" style="padding: 20px;">
          <el-form-item label="药品名称" prop="mname">
            <el-input
                v-model="currentMedicine.mname"
                required
                placeholder="请输入药品名称"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item label="药品功效" prop="mefficacy">
            <el-input
                type="textarea"
                v-model="currentMedicine.mefficacy"
                required
                placeholder="请输入药品功效"
                rows="4"
            ></el-input>
...
export default {
  ...
  methods: {
    ...
    saveMedicine() {
      ...
      request[method](url, this.currentMedicine) // 确认请求是否正确
          .then(response => {
            if (response && response.code === "200") { // 检查后端返回的响应是否正确
              this.dialogVisible = false;
              this.$nextTick(() => {
                this.fetchMedicines(); // 确保更新列表
